...
export class UI {
    // @ts-ignore
    static createSpinButton(app: PIXI.Application, onClick: () => void): PIXI.Sprite & { setEnabled: (enabled: boolean) => void } {
        const texture = Loader.getTexture('spinButton');
        // @ts-ignore
        const button = new PIXI.Sprite(texture) as PIXI.Sprite & { interactive: boolean; buttonMode: boolean; setEnabled: (enabled: boolean) => void };
